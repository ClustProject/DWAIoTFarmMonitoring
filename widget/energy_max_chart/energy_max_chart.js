    ...
    promises.push(
      self.ctx.http.get(
        `/api/plugins/telemetry/${entityId.entityType}/${entityId.id}/values/timeseries?interval=${
          custom.interval
        }&agg=MAX&keys=${keys.join(',')}&startTs=${custom.current.startTs}&endTs=${custom.current.endTs}`
      )
    );
...
function createDataset() {
  ...
  self.ctx.custom.chartData = {
    ...
    datasets: [
      {
        label: t('thingplus.page.entire-view.energy-max'),
        data: getDefaultData(),
        backgroundColor: '#01bcd6',
        borderColor: '#01bcd6',
        hoverBackgroundColor: '#01bcd6',
        fill: true,
        borderWidth: 1,
...
function insertData(datas) {
  ...
  if (custom.timeUnit == 'month') {
    ...
    for (let i in datas) {
      for (let j in datas[i]['f1_watt']) {
        acc[i][moment(datas[i]['f1_watt'][j].ts).month()].push(datas[i]['f1_watt'][j].value);
      }
    }
    for (let i = 0; i < acc.length; i++) {
      for (let j = 0; j < acc[i].length; j++) {
        ...
        sum = Math.max.apply(null, acc[i][j]);
    ...
    for (let i in datas) {
      for (let j in datas[i]['f1_watt']) {
        let targetIndex = custom.labels.indexOf(moment(datas[i]['f1_watt'][j].ts).format(timeFormat));
        if (targetIndex !== -1) {
          custom.chart.data.datasets[i].data[targetIndex].y = _.round(datas[i]['f1_watt'][j].value, 1);
        }
      }
    }
  }
